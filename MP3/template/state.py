...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        zero=self.zero_loc
        if zero[0]!=2:
            switch=original[zero[0]+1][zero[1]]
            new=copy.deepcopy(original)
            new[zero[0]][zero[1]]=switch
            new[zero[0]+1][zero[1]]=0
            n_state=EightPuzzleState(new, self.goal, self.dist_from_start+1, self.use_heuristic, [zero[0]+1,zero[1]])
            nbr_states.append(n_state)
        if zero[1]!=0:
            switch=original[zero[0]][zero[1]-1]
            new=copy.deepcopy(original)
            new[zero[0]][zero[1]]=switch
            new[zero[0]][zero[1]-1]=0
            n_state=EightPuzzleState(new, self.goal, self.dist_from_start+1, self.use_heuristic, [zero[0],zero[1]-1])
            nbr_states.append(n_state)
        if zero[0]!=0:
            switch=original[zero[0]-1][zero[1]]
            new=copy.deepcopy(original)
