...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for i in neighboring_grid_locs:
            n=GridState(i, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors, mst_cache=None)
            nbr_states.append(n)
        return nbr_states
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for i in neighboring_locs:
            ...
            if i in self.goal:
                ...
                goals.remove(i)
                newgoal=tuple(goals) 
            n=GridState(i, newgoal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
            nbr_states.append(n)
        return nbr_states
    ...
    def compute_heuristic(self):
        ...
        mandist=manhattan(self.state, self.goal[0])
        for i in self.goal:
            dist=manhattan(self.state, i)
            if dist<mandist:
                mandist=dist
                nearest_goal=i
        ...
        mst=mandist+self.mst_cache[self.goal]
        return mst
    
    # 1. when length is zero 
    # 2. include cache (self.mst_cache)
    # 3. manhattan(self.state, nearest_goal) + MST(self.goal)
    # 4. is_goal I reach the goal when I have 
